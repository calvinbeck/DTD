...
def gradients(df_d2Tdx2, df_dTdt, df_d2Tdx2_error, df_dTdt_error, depth_vec,
              site, step, save_figure):
    fig, ax1 = plt.subplots(figsize=(6, 4))
    color = 'tab:red'
    ax1.set_ylabel(r'$\frac{d^2T}{dx^2}$', color=color)
    df_d2Tdx2_plot = np.abs(df_d2Tdx2).mean(axis=0)
    df_d2Tdx2_plot_error = np.abs(df_d2Tdx2_error).mean(axis=0)
    ax1.plot(depth_vec[1:-1], df_d2Tdx2_plot[1:-1], marker='o', color=color)
    ax1.plot(depth_vec[1:-1], df_d2Tdx2_plot_error[1:-1], marker='D', ls=':',
    ...
    df_dTdt_plot_error = np.abs(df_dTdt_error).mean(axis=0)
    ax2.plot(depth_vec[1:-1], df_dTdt_plot[1:-1], marker='o', color='gray',
             label='Gradient')
    ax2.plot(depth_vec[1:-1], df_dTdt_plot_error[1:-1], marker='D', ls=':',
             color='gray', alpha=0.30, label='Uncertainty on gradient')
    ax2.plot(depth_vec[1:-1], df_dTdt_plot[1:-1], marker='o', color=color)
    ax2.plot(depth_vec[1:-1], df_dTdt_plot_error[1:-1], marker='D', ls=':',
             color=color, alpha=0.30)
    ax2.tick_params(axis='y', labelcolor=color)
    ax1.set_xlabel('Depth (m)')
    ax1.set_title(r'Gradients: $\frac{d^2T}{dx^2}$ vs. $\frac{dT}{dt}$')
    y_max = np.max(np.maximum(df_dTdt_plot[1:-1], df_dTdt_plot_error[1:-1]))
    y_min = np.min(np.minimum(df_dTdt_plot[1:-1], df_dTdt_plot_error[1:-1]))
    ax2.set_ylim(0.8*y_min, 1.2*y_max)
    ax2.legend(loc='lower left', shadow=True, ncol=1)
    if save_figure:
        if not os.path.exists('saved_figures/' + site + '_figures'):
            os.makedirs('saved_figures/' + site + '_figures')
