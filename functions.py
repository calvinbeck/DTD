...
def calc_d2Tdx2(df, depth, depth_vec, T_error, Delta_x_error):
    df_d2Tdx2 = df.copy()
    df_d2Tdx2_error = df.copy()
    for height in range(len(depth)):
        if height == 0 or height+1 == len(depth):
            df_d2Tdx2[depth[height]] = np.NaN  # boundary condition
            df_d2Tdx2_error[depth[height]] = np.NaN
        else:
            ...
            df_d2Tdx2[depth[height]] = equations.d2Tdx2(df[depth[height-1]],
                                                        ...
                                                        df[depth[height+1]],
                                                        Delta_x)
            df_d2Tdx2_error[depth[height]] = equations.d2Tdx2_error(
                                                        df[depth[height-1]],
                                                        df[depth[height]],
                                                        df[depth[height+1]],
                                                        Delta_x, T_error,
                                                        Delta_x_error)
    return df_d2Tdx2, df_d2Tdx2_error


def calc_dTdt(df, depth, T_error):
    df = df.transpose()
    df_dTdt = df.copy()
    df_dTdt_error = df.copy()
    ...
    for time in range(len(timestamp)):
        if time == 0 or time+1 == len(timestamp):  # boundary condition
            ...
            df_dTdt_error[timestamp[time]] = np.NaN
        else:
            df_dTdt[timestamp[time]] = equations.dTdt(df[timestamp[time-1]],
                                                      df[timestamp[time+1]],
                                                      Delta_t)
            df_dTdt_error[timestamp[time]] = equations.dTdt_error(
                                                      df[timestamp[time-1]],
                                                      df[timestamp[time+1]],
                                                      Delta_t, T_error)
    df = df.transpose()
    df_dTdt = df_dTdt.transpose()
    df_dTdt_error = df_dTdt_error.transpose()
    return df_dTdt, df_dTdt_error


def calc_kappa_error(df_d2Tdx2, df_dTdt, df_d2Tdx2_error, df_dTdt_error,
                     depth):
    df_kappa_error = df_d2Tdx2.copy()
    for height in range(len(depth)):
        if height == 0 or height+1 == len(depth):
            df_kappa_error[depth[height]] = np.NaN
        else:
            df_kappa_error = equations.kappa_error(df_d2Tdx2, df_dTdt,
                                                   df_d2Tdx2_error,
                                                   df_dTdt_error)
    return df_kappa_error
